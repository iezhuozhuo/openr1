...
if __name__ == "__main__":
    ...
    data_path = "/extrahome0/Zhuo/output/evol_s1k/v1/evol_s1k_m.pkl"
    cnt = 0
    ...
    for dp in dps:
        ...
        # 最优solution的正确性
        all_rwd, all_details = calculate_fitness(all_solutions, gt_answer, tokenizer)
        best_rwd = all_rwd[max_fit_idx]
        best_acc = all_details[max_fit_idx]['acc_rwd']
        if best_acc >= 0.5:
            best_right += 1
        best_rwds.append(best_rwd)
        # TODO 进化之后多样性变化
    ...
    print(f"最优回复的正确率: {best_right/len(dps):.3f}, 最优回复的rwd: {np.mean(best_rwds):.3f}")
